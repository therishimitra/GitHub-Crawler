...
def openWrite(url_list, func_list):
    ...
    for url in url_list:
        ...
        dep_str = ''# stores functions that the functions is dependant on
        dep_loc_str = '' # stores if dependancy is an argument, in the body or in the return statement
        try:
            for tr in table_rows:
                ...
                for f_name in func_list:
                    if f_name in row[1] and not 'def '+f_name in row[1]: #if function name is present in string and it is not the function definition itself
                        dep_str = dep_str+f_name+' ; '

                        if row[1].startswith('def '): #if some other function is being defined
                            dep_loc_str=dep_loc_str+'Argument ;'

                        elif 'return' in row[1]:
                            dep_loc_str = dep_loc_str + 'Return ;'

                        else:
                            dep_loc_str = dep_loc_str + 'Body ;'
                ...
                if row[1].startswith('def '):
                    ...
                    updateCSV(r'C:\Users\rishi\PycharmProjects\numpy_crawler\venv\GH_Database.csv',
                              [name_str, loc_str, desc_str, param_str, ret_str,dep_str,dep_loc_str])
